...
A person link refers to an email address and might optionally have a name associated with the address. If the name is set, the name is what is displayed in the document body.
...
Returns the person's display name, if set. Returns `null` if the element displays the email address instead.
