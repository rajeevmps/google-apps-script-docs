...
A builder for `ComposeActionResponse` objects. Note: This object isn't related to compose actions that are used to extend the compose UI. Rather, this builder creates responses to an `Action` that composes draft messages when a specific UI element is selected.
...
**Throws:** Error if the constructed compose action response isn't valid.
